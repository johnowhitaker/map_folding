...
std::vector<State> collect_partial_states(int grid_size, int partition_depth) {
    int dim = 2;
    int p[2] = {grid_size, grid_size};
    int n = p[0] * p[1];

    int *bigP = new int[dim + 1];
    bigP[0] = 1;
    for (int i = 1; i <= dim; i++) {
...
int main(int argc, char *argv[]) {
    ...
    auto states = collect_partial_states(grid_size, partition_depth);
